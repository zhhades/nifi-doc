...
![putfile属性](assets/putfile属性.gif)
...
![连接处理器](assets/连接处理器.gif)

启动GetFile

![启动getfile](assets/启动getfile处理器.gif)
...
![](assets/360截图18430710101118138.jpg)
...
![putfile自连接](assets/putfile自连接.gif)
...
![修改putfile配置项](assets/修改putfile配置项.gif)
...
![img](assets/provenance-table.png)
...
首先NIFI是基于流式处理数据，那么数据就会有移动方向，连接关系就是帮助我们路由数据的。简单的我们可以把一个组件的连接关系连向下一个组件，但有的时候我们不关心某些连接，比如说failure、original等等，我们可以通过如下图设置而不用将他们连接到别的组件。 ![img](assets/常见问题.png)
